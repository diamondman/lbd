...
produces incorrect machine code.

This chapter will cover the basic structure of LLVM, the architecture of our
example backend target architecture (CPU0), and how to build your first backend
from a CPU architecture specification.
...
The creation of the CPU0 LLVM backend will show you how to write register and
instruction definitions in the Target Description files (which will be used in
the next chapter), as well as teach you important compiler concepts like
DAGs (Directed-Acyclic-Graph) and the instruction selection needed in llvm backend design.
...
to provide the llvm middle layer the information it needs to remove
